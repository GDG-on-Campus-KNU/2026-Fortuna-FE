import { contentsApi, type GenerateContentParams } from './api';
...
export const contentRepository = {
  ...
  async listWithRevalidate(
    onUpdate: (fresh: Content[]) => void,
  ): Promise<Content[]> {
    const cached = mmkvStore.getContents();

    contentsApi
      .list()
      .then((fresh) => {
        mmkvStore.setContents(fresh);
        onUpdate(fresh);
      })
      .catch((err) => {
  ...
  async getDetail(id: string): Promise<Content> {
    const fresh = await contentsApi.detail(id);
    mmkvStore.upsertContent(fresh);
    return fresh;
  },

  // 서버 삭제 + 캐시 정리 + 오프라인 엔트리 정리.
  // 오프라인 파일 자체 삭제는 P2-7 도입 시 offline 엔티티에서 처리한다.
  async remove(id: string): Promise<void> {
    await contentsApi.remove(id);
    mmkvStore.removeContent(id);
  ...
  async generate(
    ...
    optimistic: Omit<Content, 'id' | 'status' | 'audioUrl' | 'createdAt'>,
  ): Promise<{ jobId: string; contentId: string }> {
    const result = await contentsApi.generate(params);
    mmkvStore.upsertContent({
      ...optimistic,
      id: result.contentId,
      status: 'generating',
      audioUrl: null,
